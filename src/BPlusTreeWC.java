...
public class BPlusTreeWC extends StringTree {
    ...
    private void BTreeSplitChild(@NotNull Node x, int i) {
        ...
        y.n = t - 1;
        System.arraycopy(x.c, i + 1, x.c, i + 2, x.n - i + 1);
        x.c[i + 1] = z;
        for (int j = x.n; j >= i; j--)
    ...
    private void BTreeInsertNonFull(@NotNull Node x, String k, String value) {
        int i = x.n;
        if (x.leaf) {
            while (i >= 0 && k.compareTo(x.entries[i].key) < 0) {
                x.entries[i + 1].key = x.entries[i].key;
                x.entries[i + 1].setValue(x.entries[i].getValue());
                i--;
            }
            x.entries[i + 1].key = k;
            x.entries[i + 1].setValue(value);
            x.n++;
        } else {
            while (i >= 0 && k.compareTo(x.entries[i].key) < 0)
                i++;
    ...
    private class Node {
        boolean leaf;
        int n = 0;
        BTreeEntry[] entries = new BTreeEntry[t];
        Node[] c = new Node[t];
    }
